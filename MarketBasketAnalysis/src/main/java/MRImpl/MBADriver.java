...
public class MBADriver extends Configured implements Tool{
    private static final Logger THE_LOGGER = LoggerFactory.getLogger(MBADriver.class);
    ...
    public int run(String args[]) throws Exception {
        ...
        job.getConfiguration().setInt("number.of.items.each.pair", numberOfPairs);

        // job.setJarByClass(MBADriver.class);
        // add jars to distributed cache
        if (HadoopUtil.pathExists(new Path("/lib/"), FileSystem.get(job.getConfiguration()))){
